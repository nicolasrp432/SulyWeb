-- =====================================================================
-- Phase 1 — Foundation: consolidate booking model, fix RLS, settings,
-- anti-double-booking, public availability without PII.
-- Safe / non-destructive: keeps legacy tables, only adds columns &
-- policies. Idempotent where possible.
-- =====================================================================
...
-- Backfill any data that may already live in bookings_admin_meta
...
-- 7. RLS on bookings — lock down to authenticated admins, add UPDATE/DELETE
--    (previously: public SELECT exposed all client PII, and no UPDATE/DELETE
--     policy silently blocked every admin edit)
...
create policy "bookings admin insert" on public.bookings for insert to authenticated with check (true);
...
-- 8. booking_services — only admins read/write; public path goes through RPC
...
-- 9. Public availability — a SECURITY DEFINER function (documented Supabase
--    pattern) exposing ONLY slot occupancy for a given location/date, never PII.
...
-- 10. Public booking RPC — atomic creation with server-side conflict &
--     block validation. Lets anonymous clients book without any direct
--     read/write access to the bookings table.
...
begin
  ...
  if p_location_id is null or p_booking_date is null then
    raise exception 'LOCATION_DATE_REQUIRED';
  end if;
  ...
  if exists (
    ...
    where location_id = p_location_id
      ...
      and booking_time = v_time
      and status <> 'cancelled'
  ) then
    raise exception 'SLOT_TAKEN';
  end if;

  -- slot blocked by admin (full-day or time-range block)?
  if exists (
    ...
    where b.block_date = p_booking_date
      ...
      and (
        ...
        or (v_time::time >= b.start_time and v_time::time < coalesce(b.end_time, time '23:59'))
      )
  ) then
  ...
  insert into public.bookings(
    location_id, booking_date, booking_time,
    client_name, client_phone, client_email, notes,
    status, origin
  ) values (
    p_location_id, p_booking_date, v_time,
    trim(p_client_name), trim(p_client_phone), nullif(p_client_email, ''), nullif(p_notes, ''),
    'confirmed', 'online'
  )
...
grant execute on function public.create_public_booking(
  uuid, date, text, text, text, text, text, uuid[]
